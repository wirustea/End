...
public:
	...
	struct CBChangesEveryInstanceDrawing
	{
		...
		DirectX::XMMATRIX carworld;		// 车体世界矩阵，用来计算车灯
	};
	...
	{
		int textureUsed;				// 启用纹理
		int reflectionEnabled;			// 开启反射
		int refractionEnabled;			// 开启折射
		float eta;						// 空气/介质折射率
		DirectX::XMVECTOR fogColor;		// 雾颜色
		int fogEnabled;					// 开启雾效
		float fogStart;					// 雾开始范围
		float fogRange;					// 雾范围
		int shadow;						// 开启阴影
	};
	...
	struct CBChangesEveryFrame
	{
		...
		DirectX::XMMATRIX projView;		// 虚拟点光源视图矩阵
		DirectX::XMMATRIX carView;		// 车体光源视图矩阵
	...
	struct CBChangesOnResize
	{
		...
		DirectX::XMMATRIX projProj;		// 虚拟点光源投影矩阵
		DirectX::XMMATRIX carProj;		// 车体光源投影矩阵
	};

	struct CBChangesRarely
	{
		CarLight carLight[2];			// 车灯
...
public:
	...
	ComPtr<ID3D11ShaderResourceView> m_pProjTexture;						// 虚拟点光源阴影纹理
	ComPtr<ID3D11ShaderResourceView> m_pCarLightTexture;					// 车灯阴影纹理
};
...
void BasicEffect::SetRenderDefault(ID3D11DeviceContext * deviceContext, RenderType type)
{
	...
	deviceContext->PSSetSamplers(0, 1, RenderStates::SSAnistropicWrap.GetAddressOf());  //各向异性过滤器
	deviceContext->PSSetSamplers(1, 1, RenderStates::SSStateClamp.GetAddressOf());
	deviceContext->OMSetDepthStencilState(nullptr, 0);
	deviceContext->OMSetBlendState(nullptr, nullptr, 0xFFFFFFFF);

	BasicEffect::SetShadow(false);
}

void BasicEffect::SetRenderShadow(ID3D11DeviceContext * deviceContext, RenderType type)
{
	BasicEffect::SetRenderDefault(deviceContext, type);
	deviceContext->OMSetDepthStencilState(RenderStates::DSSLessEqual.Get(), 0);
	BasicEffect::SetShadow(true);
}

void XM_CALLCONV BasicEffect::SetWorldMatrix(FXMMATRIX W)
{
...
void XM_CALLCONV BasicEffect::SetCarWorldMatrix(FXMMATRIX W)
{
	auto& cBuffer = pImpl->m_CBInstDrawing;
	cBuffer.data.carworld = XMMatrixTranspose(W);
	pImpl->m_IsDirty = cBuffer.isDirty = true;
}
...
void XM_CALLCONV BasicEffect::SetProjViewMatrix(FXMMATRIX V)
{
	auto& cBuffer = pImpl->m_CBFrame;
	cBuffer.data.projView = XMMatrixTranspose(V);
	pImpl->m_IsDirty = cBuffer.isDirty = true;
}

void XM_CALLCONV BasicEffect::SetProjProjMatrix(FXMMATRIX P)
{
	auto& cBuffer = pImpl->m_CBOnResize;
	cBuffer.data.projProj = XMMatrixTranspose(P);
	pImpl->m_IsDirty = cBuffer.isDirty = true;
}

void XM_CALLCONV BasicEffect::SetCarViewMatrix(FXMMATRIX V)
{
	auto& cBuffer = pImpl->m_CBFrame;
	cBuffer.data.carView = XMMatrixTranspose(V);
	pImpl->m_IsDirty = cBuffer.isDirty = true;
}

void XM_CALLCONV BasicEffect::SetCarProjMatrix(FXMMATRIX P)
{
	auto& cBuffer = pImpl->m_CBOnResize;
	cBuffer.data.carProj = XMMatrixTranspose(P);
	pImpl->m_IsDirty = cBuffer.isDirty = true;
}
...
void BasicEffect::SetCarLight(size_t pos, const CarLight & carLight)
{
	auto& cBuffer = pImpl->m_CBRarely;
	cBuffer.data.carLight[pos] = carLight;
	pImpl->m_IsDirty = cBuffer.isDirty = true;
}
...
void BasicEffect::SetProjTexture(ID3D11ShaderResourceView * texture)
{
	pImpl->m_pProjTexture = texture;
}

void BasicEffect::SetCarLightTexture(ID3D11ShaderResourceView * texture)
{
	pImpl->m_pCarLightTexture = texture;
}
...
void BasicEffect::SetShadow(bool isOn)
{
	auto& cBuffer = pImpl->m_CBStates;
	cBuffer.data.shadow = isOn;
	pImpl->m_IsDirty = cBuffer.isDirty = true;
}
...
void BasicEffect::Apply(ID3D11DeviceContext * deviceContext)
{
	...
	deviceContext->PSSetShaderResources(2, 1, pImpl->m_pProjTexture.GetAddressOf());
	deviceContext->PSSetShaderResources(3, 1, pImpl->m_pCarLightTexture.GetAddressOf());
