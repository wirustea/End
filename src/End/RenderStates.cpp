...
ComPtr<ID3D11SamplerState> RenderStates::SSStateWrap			= nullptr;
ComPtr<ID3D11SamplerState> RenderStates::SSStateClamp			= nullptr;
...
void RenderStates::InitAll(ID3D11Device * device)
{
	...
	HR(device->CreateSamplerState(&samplerDesc, SSStateWrap.GetAddressOf()));

	//第九,填充采样形容结构体,并且创建CLAMP的采样状态
	samplerDesc.AddressU = D3D11_TEXTURE_ADDRESS_CLAMP;
	samplerDesc.AddressV = D3D11_TEXTURE_ADDRESS_CLAMP;
	samplerDesc.AddressW = D3D11_TEXTURE_ADDRESS_CLAMP;
	HR(device->CreateSamplerState(&samplerDesc, SSStateClamp.GetAddressOf()));
